...
表单验证使用管道操作，所以你需要对[PIPE管道](https://doc.houdunren.com/nest.js/7 管道PIPE.html)有所了解。
...
创建**dto**(Data Transfer Object) 文件 **auth/dto/create-user.dto.ts** ，用于定义验证规则
...
一般情况下更新与添加的**Dto**是类似的，这时可以使用 [类型映射](https://docs.nestjs.com/openapi/mapped-types) 优化代码，类型映射内部使用了 [@nestjs/mapped-types](https://github.com/nestjs/mapped-types) 包。
...
下面是 **UpdateArticleDto** 继承 **CreateArticleDto**，并将所有属性设置为可选，更多使用请参考[类型映射](https://docs.nestjs.cn/9/techniques?id=映射类型)文档。
...
向军大叔定义一些开发时常用的规则，因为验证使用 [class-validator](https://github.com/typestack/class-validator) 所以要按其要求配置。
