...
[官网文档](https://docs.nestjs.com/security/authentication) 对新手来讲由于内容较多，可能不太好理解，本章向军大叔教大家快速搭建JWT。
...
我们将验证的 **@UseGuards(AuthGuard('jwt'))** 代码通过 [装饰器](https://docs.nestjs.cn/9/customdecorators) 进行简化操作。
...
装饰器 **decorator/auth.decorator.ts** 内容如下，使用 [装饰器聚合](https://docs.nestjs.cn/9/customdecorators?id=装饰器聚合) 功能完成。

你可以把[装饰器聚合](https://docs.nestjs.cn/9/customdecorators?id=装饰器聚合) 理解为应用多个装饰器。
...
![An image](../../public/nestjs/03.png)

### 用户验证

创建 **admin.guard.ts** 守卫，验证用户 ID为1时，身份验证通过。

- 如果当前登录用户 **id** 为**1** 时通过验证
